# Copyright (c) 2013, hadeel.milad@ard.ly and contributors
			...
			for wh in sorted(iwb_map[item]):
				for batch in sorted(iwb_map[item][wh]):
					...
					if qty_dict.opening_qty or qty_dict.in_qty or qty_dict.out_qty or qty_dict.bal_qty:
						data.append([item, item_map[item]["item_name"], item_map[item]["description"], wh, batch, item_map[item]["expiry_date"],
...
def get_columns(filters):
	...
	columns = [_("Item") + ":Link/Item:100"] + [_("Item Name") + "::150"] + [_("Description") + "::150"] + \
		[_("Warehouse") + ":Link/Warehouse:100"] + [_("Batch") + ":Link/Batch:100"] + [_("Expiry Date") + "::150"] + \
